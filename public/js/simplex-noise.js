(function() {
     ...
     * wave
     */
    const {PI, abs, atan2, ceil, floor, log, max, min, pow, round, sqrt, tan} = Math
    ...
    if(canvas === null) {
        canvas = document.createElement("hero");
        canvas.id = "hero";
    ...
    let _lastCanvasTime,
        ...
        _logMouseEvents = false,
        _mouseUpdateTimeThreshold = 12,
        mouseUpdate = -Infinity,
        mousePos = null,
        mousePosPrev = null
    
    function updateMouse(e, eventName) // Modified from p5.js
    {
        if(_logMouseEvents) console.log("Mouse event", eventName, e)
        ...
        if(!e) {
            if(_logMouseEvents) console.log("Missing event data")
        ...
        if(mouseUpdate > 0 && _mouseUpdate - mouseUpdate < _mouseUpdateTimeThreshold) {
            if(_logMouseEvents) console.log("Skipping mouse event, are the dev tools open?")
    ...
    function _draw(timestamp)
    {
        ...
        if(!_lastCanvasTime) _lastCanvasTime = timestamp
        ...
        if(_canvasOptions.drawAndStop) return
    ...
    {
        width = canvas.width = _canvasOptions.width !== null
            ? _canvasOptions.width
            : window.innerWidth
        
        height = canvas.height = _canvasOptions.height !== null
        ...
        if("onResize" in window) window.onResize()
    }
    
    function clear(x, y, w, h)
    {
        if(x !== undefined && typeof x === "number") ctx.clearRect(x, y, w, h)
        else if(_canvasOptions.centered && _canvasCurrentlyCentered /*  && x !== null */) ctx.clearRect(-width_half, -height_half, width, height)
    ...
    function background(a)
    {
        ...
        if(typeof a !== "number") fillStyle(a)
        
        if(_canvasOptions.centered && _canvasCurrentlyCentered) ctx.fillRect(-width_half, -height_half, width, height)
    ...
    function fillStyle(...args)
    {
        if(args.length === 1)
        {
            ...
            if(typeof a === "string" || a instanceof CanvasGradient || a instanceof CanvasPattern) ctx.fillStyle = a
    ...
    {
        if(typeof w === "number") ctx.lineWidth = w
    ...
    function strokeStyle(...args)
    {
        ...
        {
            let [a] = args
            if(typeof a === "string" || a instanceof CanvasGradient) ctx.strokeStyle = a
    ...
    {
        let r1 = 255, b1 = 255, g1 = 255, a1 = 1, r2 = 0, g2 = 0, b2 = 0, a2 = 1, t = 0.5
        
        if(args.length === 3) {
            if(Array.isArray(args[0]) && Array.isArray(args[1])) return lerpRGB(...args[0], ...args[1], args[2])
        ...
        } else if(args.length === 7) [r1, g1, b1, r2, g2, b2, t] = args
        else if(args.length === 9) [r1, g1, b1, a1, r2, g2, b2, a2, t] = args
        else if(args.length === 2 && Array.isArray(args[0])) {
            if(args[0].length === 2) return lerpRGB(...args[0], args[1])
            
            // TODO: Allow (possibly weighted) lerping between n-count RGBs at specified positions
            
        } else return { r: 127.5, g: 127.5, b: 127.5, a: 1 }
        
        let r = lerp(r1, r2, t), g = lerp(g1, g2, t), b = lerp(b1, b2, t), a = lerp(a1, a2, t)
    ...
    function hsl(hue, sat, light, alpha = 1)
    {
        ...
        {
            if(Array.isArray(hue)) [hue, sat, light, alpha = alpha] = hue
            else if("h" in hue) ({ h: hue, s: sat, l: light, a: alpha = alpha } = hue)
        }
    ...
    function stroke(...args)
    {
        ...
        if(args.length)
        {
            if(args[0] instanceof Path2D) path = args.shift()
    ...
    function createLinearGradient(x1 = -100, y1 = -100, x2 = 100, y2 = 100, stops = [])
    {
        ...
        if(stops && Array.isArray(stops) && stops.length) {
            stops.forEach(stop => {
                ...
                try {
                    if(Array.isArray(stop)) grad.addColorStop(stop[0], stop[1])
                    
                    // { offset: number, color: string }
                    else if(stop.offset && stop.color) grad.addColorStop(stop.offset, stop.color)
    ...
    function drawImage(img, ...inputArgs)
    {
        ...
        if(inputArgs.length === 0) finalArgs.push(0, 0)
        else {
        ...
        if(img && img instanceof AlcaImage) img = img.image
        
        if(img) ctx.drawImage(img, ...finalArgs)
    }
    ...
    {
        if(typeof x === "number") ctx.translate(x, y)
        else if("x" in x) ctx.translate(x.x, x.y)
    }
    ...
    {
        rot = rot % TAU
        if(offsetX === undefined) ctx.rotate(rot)
    ...
    function compensateCanvas()
    {
        ...
        if(offX || offY) translate(offX, offY)
    }
    ...
    const compOper = {
        ...
        destinationIn: "destination-in", destinationOut: "destination-out",
        destinationAtop: "destination-atop", lighter: "lighter", copy: "copy", xor: "xor",
        ...
        source: {over: "source-over", in: "source-in", out: "source-out", atop: "source-atop"},
        destination: {over: "destination-over", in: "destination-in", out: "destination-out", atop: "destination-atop"},
        light: {hard: "hard-light", soft: "soft-light"}
    }
    
    // https://developer.mozilla.org/en-US/docs/Web/API/CanvasRenderingContext2D/globalCompositeOperation
    function compositeOperation(type = compOper.default) {ctx.globalCompositeOperation = type}
    
    function filter(filterFuncs = "none") {ctx.filter = filterFuncs || "none"}
    
    function beginPath() {ctx.beginPath()}
    
    function moveTo(x, y)
    {
        if(typeof x === "number") ctx.moveTo(x, y)
        else if(isVectorish(x)) ctx.moveTo(x.x, x.y)
    }
    
    function lineTo(x, y)
    {
        if(typeof x === "number") ctx.lineTo(x, y)
        else if(isVectorish(x)) ctx.lineTo(x.x, x.y)
    }
    ...
    function quadraticCurveTo(cpX, cpY, x, y)
    {
        ...
        if(typeof cpX === "number") {
            ...
            if(typeof x === "number") b = [x, y]
            else if("x" in x) b = x.xy
            
        } else if("x" in cpX) {
            a = cpX.xy
            
            if(typeof cpY === "number") b = [cpY, x]
            else if("x" in cpY) b = cpY.xy
        }
    ...
    function bezierCurveTo(cp1X, cp1Y, cp2X, cp2Y, x, y)
    {
        ...
        if(typeof cp1X === "number") {
            ...
            if(typeof cp2X === "number") {
                ...
                if(typeof x === "number") c = [x, y]
                else if("x" in x) c = x.xy
                
            } else if("x" in cp2X) {
                b = cp2X.xy
                
                if(typeof cp2Y === "number") c = [cp2Y, x]
                else if("x" in cp2Y) c = cp2Y.xy
            }
            ...
            if(typeof cp1Y === "number") {
                ...
                if(typeof cp2X === "number") c = [cp2X, cp2Y]
                else if("x" in cp2X) c = cp2X.xy
            }
        }
    ...
    function vertices(...verts)
    {
        ...
        else if(verts.length === 1 && Array.isArray(verts[0])) verts = verts[0]
        ...
        for(let i = 0; i < verts.length; i++) {
            let n = verts[i], x = 0, y = 0
            
            if(Array.isArray(n)) [x, y] = n
            else if(isVectorish(n)) ({ x, y } = n)
            
            if(!shouldMoveFirst || i !== 0) lineTo(x, y)
            else moveTo(x, y)
        }
    }
    ...
    function circle(x, y, rX, rY)
    {
        ...
        if(isVectorish(rX)) ({ x: rX, y: rY } = rX)
        ...
        if(rY !== undefined) ellipse(x, y, rX, rY)
        else {
            if(rX < 0) rX = 0
    ...
    function genRegularPolygon(sides = 3, radius = 50, rotation = 0)
    {
        let iSizes = 1 / sides * TAU,
            data = {sides, radius, rotation, points: []}
    ...
    {
        if(_codepenIDRegex.test(window.location.href)) return _codepenIDRegex.exec(window.location.href)[1]
        else {
    ...
    function isPreviewEmbed() {return location.href.includes('/fullcpgrid/')}
    ...
    loadImage.alca = function(urlPart) {return loadImage("https://alca.tv/static/" + urlPart)}
    loadImage.alca.pen = function(urlPart) {return loadImage.alca("codepen/" + urlPart)}
    loadImage.alca.pen._ = function(urlPart) {return loadImage.alca.pen(`pens/${getCodePenID()}/${urlPart}`)}
    ...
    loadVideo.alca = function(urlPart) {return loadVideo("https://alca.tv/static/" + urlPart)}
    
    loadVideo.alca.pen = function(urlPart) {return loadVideo.alca("codepen/" + urlPart)}
    
    loadVideo.alca.pen._ = function(urlPart) {return loadVideo.alca.pen(`pens/${getCodePenID()}/${urlPart}`)}
    
    function loadData(url) {return fetch(url)}
    
    loadData.alca = function(urlPart) {return loadData("https://alca.tv/static/" + urlPart)}
    
    function loadText(url) {return loadData(url).then(res => res.text())}
    
    loadText.alca = function(urlPart) {return loadText("https://alca.tv/static/" + urlPart)}
    
    function loadJSON(url) {return loadData(url).then(res => res.json())}
    
    loadJSON.alca = function(urlPart) {return loadJSON("https://alca.tv/static/" + urlPart)}
    ...
    function getImageData(img, ...args)
    {
        if(img instanceof Image) {
            ...
            if(args.length) data = ctx.getImageData(...args)
            else data = ctx.getImageData(0, 0, img.width, img.height)
            
            return Object.assign(data, { canvas, ctx })
            
        } else return ctx.getImageData(img, ...args)
    }
    ...
    function putImageData(imageData, x, y, sourceX, sourceY, sourceWidth, sourceHeight)
    {
        ...
        if(isVectorish(sourceWidth)) ({ x: sourceWidth, y: sourceHeight } = sourceWidth)
    ...
    {
        if(Array.isArray(low)) return low[floor(Math.random() * low.length)]
    ...
    {
        if(typeof start !== "number") return Vector.lerp(start, stop, amt)
    ...
    function distSq(x1, y1, x2, y2)
    {
        ...
        if(isVectorish(x2)) ({ x: x2, y: y2 } = x2)
    ...
    class AlcaImage
    {
        ...
        static load(url) {return new AlcaImage({ promise: loadImage(url) })}
    }
    ...
    class Vector
    {
        ...
        static center() {return new Vector(width_half, height_half)}
        
        static from(v, ...args) {
            if(v === undefined) return new Vector()
            else if(Array.isArray(v)) return new Vector(...v)
            else if(typeof v === "object") return new Vector(v.x, v.y, v.z)
            else if(typeof v === "number") return new Vector(v, ...args)
        }
        
        static fromAngle(angle, mult = 1) {return new Vector(cos(angle), sin(angle)).mult(mult)}
        
        static fa(...args) {return Vector.fromAngle(...args)}
        
        static random2D(angle = true, mult = 1) {
            let v
            
            if(angle === true) v = Vector.fromAngle(random(TAU))
            else v = new Vector(random(-1, 1), random(-1, 1))
            
            if(typeof angle === "number") v.mult(angle)
            else if(mult !== 1) v.mult(mult)
        ...
        static lerp(start, stop, amt = 0.5, apply = false) {
            ...
            if(apply) return start.set(x, y, z)
            
            return new Vector(x, y, z)
        }
        
        static average(vectors, ...rest) {
            if(rest.length) vectors = [vectors, ...rest]
        ...
        equals(vec, tolerance = 0) {
            if(tolerance === 0) return this.x === vec.x && this.y === vec.y
            
            return abs(this.x - vec.x) < tolerance && abs(this.y - vec.y) < tolerance
        }
        
        equals3D(vec = {}) {return this.x === vec.x && this.y === vec.y && this.z === vec.z}
        ...
        setYX(...args) {return this.setXY(...args)}
        ...
        setZY(...args) {return this.setYZ(...args)}
        ...
        setZX(...args) {return this.setXZ(...args)}
        ...
        addXY(x, y = x) {return this.addX(x).addY(y)}
        addYX(y, x = y) {return this.addXY(x, y)}
        addYZ(y, z = y) {return this.addY(y).addZ(z)}
        addZY(z, y = z) {return this.addYZ(y, z)}
        addZX(z, x = z) {return this.addZ(z).addX(x)}
        addXZ(x, z = x) {return this.addZX(x, z)}
        addXYZ(...args) {return this.add(...args)}
        ...
        subXY(x, y = x) {return this.subX(x).subY(y)}
        subYX(y, x = y) {return this.subXY(x, y)}
        subYZ(y, z = y) {return this.subY(y).subZ(z)}
        subZY(z, y = z) {return this.subYZ(y, z)}
        subZX(z, x = z) {return this.subZ(z).subX(x)}
        subXZ(x, z = x) {return this.subZX(x, z)}
        subXYZ(...args) {return this.sub(...args)}
        ...
        multXY(x, y = x) {return this.multX(x).multY(y)}
        multYX(y, x = y) {return this.multXY(x, y)}
        multYZ(y, z = y) {return this.multY(y).multZ(z)}
        multZY(z, y = z) {return this.multYZ(y, z)}
        multZX(z, x = z) {return this.multZ(z).multX(x)}
        multXZ(x, z = x) {return this.multZX(x, z)}
        multXYZ(...args) {return this.mult(...args)}
        ...
        divXY(x, y = x) {return this.divX(x).divY(y)}
        divYX(y, x = y) {return this.divXY(x, y)}
        divYZ(y, z = y) {return this.divY(y).divZ(z)}
        divZY(z, y = z) {return this.divYZ(y, z)}
        divZX(z, x = z) {return this.divZ(z).divX(x)}
        divXZ(x, z = x) {return this.divZX(x, z)}
        divXYZ(...args) {return this.div(...args)}
        ...
        rotateAbout(angle, point) {return this.sub(point).rotate(angle).add(point)}
        
        magSq() {return this.x * this.x + this.y * this.y}
        magSq3D() {return this.x * this.x + this.y * this.y + this.z * this.z}
        mag() {return Math.sqrt(this.magSq())}
        mag3D() {return Math.sqrt(this.magSq3D())}
        
        normalize(mag = this.mag()) {return mag === 0 ? this : this.div(mag)}
        normalize3D(mag = this.mag3D()) {return mag === 0 ? this : this.div(mag)}
        setMag(mag) {return this.normalize().mult(mag)}
        setMag3D(mag) {return this.normalize3D().mult(mag)}
        ...
        dist3D(v) {return v.copy().sub(this).mag3D()}
        
        lerp(stop, amt) {return Vector.lerp(this, stop, amt, true)}
        ...
        lerpX(stop, amt) {return this._lerpPart(stop, amt, "x")}
        lerpY(stop, amt) {return this._lerpPart(stop, amt, "y")}
        lerpZ(stop, amt) {return this._lerpPart(stop, amt, "z")}
        lerpXY(stop, amt) {return this.lerpX(stop, amt).lerpY(stop, amt)}
        lerpYX(stop, amt) {return this.lerpXY(stop, amt)}
        lerpYZ(stop, amt) {return this.lerpY(stop, amt).lerpZ(stop, amt)}
        lerpZY(stop, amt) {return this.lerpYZ(stop, amt)}
        lerpZX(stop, amt) {return this.lerpZ(stop, amt).lerpX(stop, amt)}
        lerpXZ(stop, amt) {return this.lerpZX(stop, amt)}
    ...
    function linearTween(t = 0.5, b = 0, c = 1, d = 1) {return c * t / d + b;}
    /* quadratic easing in */
    function easeInQuad(t = 0.5, b = 0, c = 1, d = 1) {t /= d;return c * t * t + b;}
    /* quadratic easing out */
    function easeOutQuad(t = 0.5, b = 0, c = 1, d = 1) {t /= d;return -c * t * (t - 2) + b;}
    /* quadratic   easing in/out */
    function easeInOutQuad(t = 0.5, b = 0, c = 1, d = 1) {t /= d * 0.5;if(t < 1) return c * 0.5 * t * t + b;t--;return -c * 0.5 * (t * (t - 2) - 1) + b;}
    /* cubic easing in */
    function easeInCubic(t = 0.5, b = 0, c = 1, d = 1) {t /= d;return c * t * t * t + b;}
    /* cubic easing out */
    function easeOutCubic(t = 0.5, b = 0, c = 1, d = 1) {t /= d;t--;return c * (t * t * t + 1) + b;}
    /* cubic easing in/out */
    function easeInOutCubic(t = 0.5, b = 0, c = 1, d = 1) {t /= d * 0.5;if(t < 1) return c * 0.5 * t * t * t + b;t -= 2;return c * 0.5 * (t * t * t + 2) + b;}
    /* quartic easing in */
    function easeInQuart(t = 0.5, b = 0, c = 1, d = 1) {t /= d;return c * t * t * t * t + b;}
    /* quartic easing out */
    function easeOutQuart(t = 0.5, b = 0, c = 1, d = 1) {t /= d;t--;return -c * (t * t * t * t - 1) + b;}
    /* quartic easing in/out */
    function easeInOutQuart(t = 0.5, b = 0, c = 1, d = 1) {t /= d * 0.5;if(t < 1) return c * 0.5 * t * t * t * t + b;t -= 2;return -c * 0.5 * (t * t * t * t - 2) + b;}
    /* quintic easing in */
    function easeInQuint(t = 0.5, b = 0, c = 1, d = 1) {t /= d;return c * t * t * t * t * t + b;}
    /* quintic easing out */
    function easeOutQuint(t = 0.5, b = 0, c = 1, d = 1) {t /= d;t--;return c * (t * t * t * t * t + 1) + b;}
    /* quintic easing in/out */
    function easeInOutQuint(t = 0.5, b = 0, c = 1, d = 1) {t /= d * 0.5;if(t < 1) return c * 0.5 * t * t * t * t * t + b;t -= 2;return c * 0.5 * (t * t * t * t * t + 2) + b;}
    /* sinusoidal easing in */
    function easeInSine(t = 0.5, b = 0, c = 1, d = 1) {return -c * cos(t / d * HALF_PI) + c + b;}
    /* sinusoidal easing out */
    function easeOutSine(t = 0.5, b = 0, c = 1, d = 1) {return c * sin(t / d * HALF_PI) + b;}
    /* sinusoidal easing in/out */
    function easeInOutSine(t = 0.5, b = 0, c = 1, d = 1) {return -c * 0.5 * (cos(PI * t / d) - 1) + b;}
    /* exponential easing in */
    function easeInExpo(t = 0.5, b = 0, c = 1, d = 1) {return c * pow(2, 10 * (t / d - 1)) + b;}
    /* exponential easing out */
    function easeOutExpo(t = 0.5, b = 0, c = 1, d = 1) {return c * (-pow(2, -10 * t / d) + 1) + b;}
    /* exponential easing in/out */
    function easeInOutExpo(t = 0.5, b = 0, c = 1, d = 1) {t /= d * 0.5;if(t < 1) return c * 0.5 * pow(2, 10 * (t - 1)) + b;t--;return c * 0.5 * (-pow(2, -10 * t) + 2) + b;}
    /* circular easing in */
    function easeInCirc(t = 0.5, b = 0, c = 1, d = 1) {t /= d;return -c * (sqrt(1 - t * t) - 1) + b;}
    /* circular easing out */
    function easeOutCirc(t = 0.5, b = 0, c = 1, d = 1) {t /= d;t--;return c * sqrt(1 - t * t) + b;}
    /* circular easing in/out */
    function easeInOutCirc(t = 0.5, b = 0, c = 1, d = 1) {t /= d * 0.5;if(t < 1) return -c * 0.5 * (sqrt(1 - t * t) - 1) + b;t -= 2;return c * 0.5 * (sqrt(1 - t * t) + 1) + b;}
    ...
    function getTimeArray(timestamp = null)
    {
        if(timestamp === null) timestamp = new Date()
            ...
            if(!isNaN(parsedTimestamp)) timestamp = new Date(parsedTimestamp)
            else throw new RangeError("Invalid Date")
            
        } else if(!(timestamp instanceof Date)) throw new TypeError("Unsupported timestamp")
    ...
    {
        if("WebFont" in window === false) return Promise.reject("WebFont not available. Load using this script: https://cdnjs.cloudflare.com/ajax/libs/webfont/1.6.28/webfontloader.js")
        ...
        return new Promise((resolve, reject) => {
            ...
            if(typeof fontName === "string") providers = { google: { families: [fontName] } }
            else if(Array.isArray(fontName)) providers = { google: { families: fontName } }
            else providers = fontName
    ...
    function font(fontStr, fallbackIfDefault)
    {
        if(fontStr !== undefined) {
            ...
            if(fallbackIfDefault !== undefined && isFontDefault()) ctx.font = fallbackIfDefault
        }
    ...
    function draw(e)
    {
        ...
        let _a = 180, _b = 0, __a = [1, 29], __b = [67, 52]
        let hueA = tSide ? _a : _b, hueB = !tSide ? _a : _b
        let colorA = hsl(hueA, __a[0], __a[1])
        let colorB = hsl(hueB, __b[0], __b[1])
